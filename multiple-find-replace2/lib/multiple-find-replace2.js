...
// Global Variables
const fs = require("fs");
var path = atom.packages.getPackageDirPaths('multiple-find-replace2').toString() + '/multiple-find-replace2/multiple-find-replace2-template2.txt';
...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(
      ...
      'multiple-find-replace2:replace': () => this.replace()
    }))

    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
      'multiple-find-replace2:find': () => this.find()
    }))
  },
  ...
  find(editor) {
    ...
    var defaultText = 'Find and replace me' + atom.config.get('multiple-find-replace2.general.splitSintax') + 'Replaced!';
    fileExists = fs.existsSync(path)
    if (fileExists) {
      atom.workspace.open(path)
      if (atom.config.get('multiple-find-replace2.general.displayNotifications')) {
      ...
      fs.readFile(path, function (err, data) {
        if (err) {
          fs.writeFile(path, defaultText, function (err) {
            ...
            if (finishWithError = true && atom.config.get('multiple-find-replace2.general.displayNotifications')) {
  ...
  doreplacement(text) {
    ...
    if (fileExists) {
      ...
      for (var i = 0; i < linesQuantity; i++) {
        if (lines[i].includes(atom.config.get('multiple-find-replace2.general.splitSintax'))) {
          line = lines[i].split(atom.config.get('multiple-find-replace2.general.splitSintax'))
          line[0]
          line[1]
          if (atom.config.get('multiple-find-replace2.general.matchCase')) {

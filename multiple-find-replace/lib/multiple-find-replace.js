...
// Global Variables
const fs = require("fs");
var path = atom.packages.getPackageDirPaths('multiple-find-replace').toString() + '/multiple-find-replace/multiple-find-replace-template1.txt';
...
export default {
  ...
  activate() {
	...
	this.subscriptions.add(
      atom.commands.add('atom-workspace', {
      'multiple-find-replace:chinese': () => this.chinese()
    }))

	this.subscriptions.add(
      atom.commands.add('atom-workspace', {
      'multiple-find-replace:spanish': () => this.spanish()
    }))

	this.subscriptions.add(
      atom.commands.add('atom-workspace', {
      'multiple-find-replace:hindi': () => this.hindi()
    }))
...
hindi(editor){
console.log("HI");
path = setCharAt(path, path.length-5,'5');
},
spanish(editor){
console.log("HI");
path = setCharAt(path, path.length-5,'4');
},
  chinese(editor){
console.log("china");
path = setCharAt(path, path.length-5,'1');
},
  find(editor) {
